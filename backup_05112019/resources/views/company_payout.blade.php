        ...
        <thead>
          ...
          @foreach($payouts as $row)
            <tr>
              ...
              <td>
              {{ $row->payout_id }}  <span class="lang-chang-label">  ({{ $row->company_payout_preference->currency_code }})</span>
              </td>
              <td>
              ...
              <li class="dropdown-trigger list-unstyled">
                  ...
                  <ul data-sticky="true" data-trigger="#payout-options-{{ $row->id }}" class="tooltip tooltip-top-left list-unstyled dropdown-menu" aria-hidden="true">
                    <li>
                      <a rel="nofollow" data-method="post" class="link-reset menu-item" href="{{ url('/') }}/company/payout_delete/{{ $row->id }}">{{ trans('messages.account.remove') }}</a>
                    </li>
                    <li>
                      <a rel="nofollow" data-method="post" class="link-reset menu-item" href="{{ url('/') }}/company/payout_default/{{ $row->id }}">{{ trans('messages.account.set_default') }}</a>
                    </li>
                  </ul>
              </li>
              @endif        
            ...
            <tr id="add_payout_method_section">
              <td colspan="4">
                ...
                <span class="text-muted lang-left">
                  &nbsp;
                  Stripe, <span>PayPal, </span><span class="lang-left">etc...</span>
                </span>
              </td>
            </tr>
          </tfoot>
...
<div id="taxes"></div>
  ...
  <div class="flash-container payout-flash-message" id="popup1_flash-container"> </div>
  <form class="modal-add-payout-pref" method="post" id="address">
    ...
    <div class="panel-footer payout_footer">
      <input type="submit" value="{{ trans('messages.account.next') }}" class="btn btn-primary payout-btn" id="payout-next">
    </div>
  </form>
</div>
</div>
...
<div class="modal poppayout perferenace_payout" id="payout_popup2" aria-hidden="false" style="padding-left: 0 !important;" tabindex="-1">
  ...
  <div class="flash-container payout-flash-message" id="popup2_flash-container"> </div>
  ...
  <div class="panel-footer payout_footer">
      <input type="submit" value="{{ trans('messages.account.next') }}" id="select-payout-method-submit" class="btn btn-primary payout-btn" data-target="">
  </div>
</form>
...
<div class="modal poppayout" id="payout_popup3" aria-hidden="false" style="" tabindex="-1">
  ...
  <div class="flash-container hide payout-flash-message" id="popup3_flash-container"><div class="alert alert-error alert-error alert-header"><a class="close alert-close" href="javascript:void(0);"></a><i class="icon alert-icon icon-alert-alt"></i>{{trans('messages.account.valid_email')}}</div>  </div>  

  <form method="post" id="payout_paypal" action="{{ url('company/payout_preferences') }}" accept-charset="UTF-8">
  {!! Form::token() !!}
  ...
  <div class="panel-footer payout_footer">
    <input type="submit" value="{{ trans('messages.account.submit') }}" id="modal-paypal-submit" class="btn btn-primary payout-btn">
  </div>
</form>
</div>
</div>
...
<div id="modal-add-payout-set-address" class="modal-content">
  ...
  <form method="post" id="payout_stripe" action="{{ url('company/update_payout_preferences') }}" accept-charset="UTF-8" enctype="multipart/form-data">
  {!! Form::token() !!}
  ...
  <div class="panel-body panel-body-payout" ng-init="payout_country={{json_encode(old('country') ?: '')}};payout_currency={{json_encode(old('currency') ?: '')}};iban_supported_countries = {{json_encode($iban_supported_countries)}};branch_code_required={{json_encode($branch_code_required)}};country_currency={{json_encode($country_currency)}};change_currency();mandatory={{ json_encode($mandatory)}};old_currency='{{ old('currency') ? json_encode(old('currency')) : '' }}'">
      ...
      <div ng-if="payout_country" class="routing_number_cls" ng-hide="iban_supported_countries.includes(payout_country)">
        ...
        <div class="">
          {!! Form::text('routing_number', @$company_payout_preference->routing_number, ['id' => 'routing_number', 'class' => 'form-control']) !!}
      ...
      <div class="select">
        {!! Form::select('gender', ['male' => trans('messages.profile.male'), 'female' => trans('messages.profile.female')], '', ['id' => 'user_gender', 'placeholder' => trans('messages.profile.gender'), 'class' => 'focus','style'=>'min-width:140px;']) !!}
  ...
  <div class="panel-footer payout_footer">
    <input type="submit" value="{{ trans('messages.account.submit') }}" id="modal-stripe-submit" class="btn btn-primary payout-btn">
  </div>
</form>
</div>
</div>
