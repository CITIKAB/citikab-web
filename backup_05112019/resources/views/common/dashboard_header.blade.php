...
<div class="flexbox__item flexbox__item--expand"><ul class="site-nav site-nav--flush site-nav--dark block-list push-half--bottom"><li><div class="flexbox" style="margin-top:25px;"><div class="flexbox__item one-eighth pull-left">
  ...
  @endif
    ...
    font-size: 13px;"><div class="text--normal">{{ @Auth::user()->first_name}} {{ @Auth::user()->last_name}}</div></div><div class="flexbox__item three-eighths text--right"></div></div><div id="slide-menu-account-progress" class="pro_bar" style="margin-top:20px;"><div class="soft--top milli text--left"><div class="grid"><div class="grid__item three-quarters"><strong>{{trans('messages.header.profile')}}</strong></div><div class="grid__item one-quarter text--right"><strong class="color--negative">33%</strong></div></div><div class="progress push-half--top push--bottom"><div style="width: 33%" class="progress__bar progress__bar--negative"></div></div><div><span class="micro icon icon1--circle icon_check push-half--right icon1--inactive"></span><a href="/payment" class="link--immutable">{{trans('messages.header.credit_card')}}</a></div><div><span class="micro icon icon1--circle icon_check push-half--right icon1--inactive"></span><a href="#" data-toggle="modal" data-target="#verify-email-modal" class="link--immutable">{{trans('messages.header.verify_email')}}</a></div></div></div></li><li><a href="{{ url('trip') }}" class="active">{{trans('messages.header.mytrips')}}</a></li><li><a href="{{ url('profile') }}">{{trans('messages.header.profile')}}</a></li><li><a href="{{ url('payment') }}">{{trans('messages.header.payment')}}</a></li>
...
<ul class="nav--block float--right flush hidden--portable hide-sm-760"><li class="user-flyout flyout flyout--right"><a href="#" class="flyout__origin"><span class="icon icon_profile alpha push-half--right"></span><span class="push-half--right">{{ @Auth::user()->first_name}} {{ @Auth::user()->last_name}}</span><span class="icon icon_down-arrow milli"></span></a><div class="flyout__content" style="margin:-4px 0px 0px 0px !important;padding:0px;"><ul class="site-nav site-nav--flush"><li><div class="grid"><div class="grid__item one-quarter">
@if(@Auth::user()->profile_picture->src == '')
...
@endif
</div><div class="grid__item three-quarters"><h3 class="push-half--bottom">{{ @Auth::user()->first_name}} {{ @Auth::user()->last_name}}</h3></div></div></li><li><a href="{{ url('trip') }}" class="active">{{trans('messages.header.mytrips')}}</a></li><li><a href="{{ url('profile') }}">{{trans('messages.header.profile')}}</a></li><li><a href="{{ url('sign_out')}}">{{trans('messages.header.logout')}}</a></li></ul></div></li></ul>
</div>
</div>
<!-- </div> -->
</div>
