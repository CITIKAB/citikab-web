...
class UserController extends BaseController {
    ...
    {
    	// dd('test');
        if(!$request->address1 && !$request->bank_detail)
        { 
            $data['payouts'] = PayoutCredentials::with(['payout_preference'])->where('user_id', Auth::user()->id)->orderBy('id','desc')->get();
            ...
            $data['bank_detail'] = BankDetail::where('user_id',Auth::user()->id)->first();
        ...
        else
        {
        	...
        	if(Auth::user()->company_id > 1){
            	...
            	$this->helper->flash_message('success', trans('messages.account.bank_detail_updated')); // Call flash message function
        	}else{
		        ...
		        $payout_credentials->user_id = Auth::user()->id;
