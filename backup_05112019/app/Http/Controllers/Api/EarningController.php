	...
	public function after_payment(Request $request) {
		...
		if ($validator->fails()) {
			...
			if (count($user)) {
				...
				if($trip->payment_mode=="Stripe" || $trip->payment_mode=="Stripe & Wallet")
				{
					$transaction_id = 0;

					if($trip->total_fare!=0)
					{
						$customer_id = PaymentMethod::where('user_id', $user_details->id)->first()->stripe_customer_id;
				...
				$transaction_id = $this->request_helper->get_transaction_id_from_payment_id(@$request->paykey);
			...
			$wallet = Wallet::whereUserId($user_details->id)->first();
				...
				if($request->payment_type=="Stripe")
				{
					...
					try
					{
						...
						'currency' => isset($user_details->currency_code) ? $user_details->currency_code : 'USD',
						"customer" => $customer_id,
						]
						);
