...
class ManageFareController extends Controller
{
    	...
    	if(!$_POST) {
            ...
            if($request->apply_peak == 'Yes') {
                foreach($request->peak_fare_details ?: array() as $key => $fare_rules)
                {
                    ...
                    $niceNames['peak_fare_details.'.$key.'.day'] = 'Day';
                    $niceNames['peak_fare_details.'.$key.'.start_time'] = 'Start Time';
                    $niceNames['peak_fare_details.'.$key.'.end_time'] = 'End Time';
                    $niceNames['peak_fare_details.'.$key.'.price'] = 'Price';
            ...
            if($request->apply_night == 'Yes') {
                ...
                $niceNames['night_fare_details.start_time'] = 'Start Time';
                $niceNames['night_fare_details.end_time'] = 'End Time';
                $niceNames['night_fare_details.price'] = 'Price';
            }
            ...
            if ($validator->fails()) {
                ...
                $fare_count = ManageFare::whereLocationId($request->location)->Where('vehicle_id',$request->vehicle_type)->get()->count();
            ...
            $this->helper->flash_message('success', 'Manage Fare Details Added Successfully');
    ...
    public function update(Request $request)
    {
    	if(!$_POST) {
            ...
            if ($validator->fails()) {
                ...
                $fare_count = ManageFare::whereLocationId($request->location)->Where('vehicle_id',$request->vehicle_type)->where('id','!=',$manage_fare_id)->get()->count();
            ...
            // Call flash message function
            $this->helper->flash_message('success', 'Manage Fare Details Updated Successfully');
    ...
    public function delete(Request $request)
    {
        ...
        if(!is_null($manage_fare)) {
            ...
            if($pending_request > 0) {
                $this->helper->flash_message('error', 'Trip with this Fare is pending. So Could not Delete Fare');
                return redirect('admin/manage_fare');
            }
            ...
            $manage_fare->delete();
            // Call flash message function
            $this->helper->flash_message('success', 'Manage Fare Details successfully deleted');
        }
        else {
            // Call flash message function
            $this->helper->flash_message('warning', 'Manage Fare Details already deleted');
        }
