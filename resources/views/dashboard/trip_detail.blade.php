...
<tbody>
	...
	<tr class="gamma fare-breakdown__primary-charge">
	<td colspan="2" class="text--left">{{trans('messages.dashboard.access_fee')}}</td>
	<td class="text--right">{{ $trip->currency->original_symbol }} {{ $trip->access_fee }}</td>
	</tr>
	<tr class="gamma weight--semibold separated--top">
	<td colspan="2" class="text--left">{{trans('messages.dashboard.total_fare')}}</td>
	<td class="text--right">{{ $trip->currency->original_symbol }} {{ $trip->total_fare}}</td>
	</tr>
	@if($trip->promo_amount!=0)
	<tr class="gamma fare-breakdown__primary-charge">
	<td colspan="2" class="text--left">{{trans('messages.dashboard.promo_amt')}}</td>
	<td class="text--right"> - {{ $trip->currency->original_symbol }} {{ $trip->promo_amount }}</td>
	</tr>
	@endif
	@if($trip->wallet_amount!=0)
	<tr class="gamma fare-breakdown__primary-charge">
	<td colspan="2" class="text--left">{{trans('messages.dashboard.wallet_amt')}}</td>
	<td class="text--right"> - {{ $trip->currency->original_symbol }} {{ $trip->wallet_amount }}</td>
	</tr>
	@endif	
	@if($trip->payment_mode=="Cash" || $trip->payment_mode=="Cash & Wallet" ||  $trip->wallet_amount>0 || $trip->promo_amount > 0)
	<tr class="gamma fare-breakdown__primary-charge">
	<td colspan="2" class="text--left">{{trans('messages.dashboard.paid_amt')}}</td>
	<td class="text--right">{{ $trip->currency->original_symbol }} {{ $trip->rider_paid_amount }}</td>
	</tr>
	@endif
