...
<div class=" text-center signupdrive" ng-controller="facebook_account_kit">
  <div class="join-page" >
      ...
      <div class="layout__item layout--join__hero-item soft-gutter z-30" style="padding-top:0 !important;" >
        <div class="layout" style=" margin-left: -40px;"
          ...
          <input type="hidden" name="user_type" value="Driver">
              ...
              <!-- @if(count($errors->all()))
                <p class="text-orange text--left flush--bottom text--center text-danger">{{trans('messages.user.correct')}}</p>
                <ul class="text-orange text--left push-tiny--top">
                  @foreach ($errors->all() as $error)
                    <li class="flush--bottom text-danger">{{ $error }}</li>
                  @endforeach                  
                </ul>
              @endif -->
              </div>
    ...
    <form action="{{url('/signup_driver')}}" method="post" id="form">
