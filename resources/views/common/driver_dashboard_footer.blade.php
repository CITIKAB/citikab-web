  ...
  <div class="footer-head nt_fot col-lg-12 col-md-12 col-sm-12 col-xs-12">
    <div class="layout">
      ...
      <div class="layout_item col-lg-4 col-md-4 col-sm-4 col-xs-6 sm-pull-right">
        <a href="{{ url('signup_driver') }}" style="width: 165px;overflow: hidden;text-overflow: ellipsis;" class="btn btn--reverse-outline">{{trans('messages.footer.drive')}}</a>
      </div>
    </div>
  </div>
</div>
...
<!-- <div class="col-lg-4 col-sm-4 col-md-4 col-xs-12 social-icons" style="    margin-top: -30px;">
  @for($i=0; $i<count($join_us); $i++)
      @if($join_us[$i]->value)
        <a href="{{ $join_us[$i]->value }}" target="_blank"> 
        <span class="fa fa-{{ str_replace('_','-',$join_us[$i]->name) }}"></span>
