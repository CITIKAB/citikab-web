...
@section('main')
<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 flexbox__item four-fifths page-content driver_payment" style="padding:0px;" ng-controller="payment" ng-cloak><div class="page-lead separated--bottom  text--center text--uppercase"><h1 class="flush-h1 flush">{{trans('messages.header.payment')}}</h1>
</div>
...
<div class="pull-left">
<small>{{trans('messages.driver_dashboard.total_earnings')}}</small>
<label class="pull-left full-width" style="    font-weight: normal;
    font-size: 40px;">
    {{session('symbol')}} {{$total_earnings}}
</label>
</div>
...
<small class="pull-right pull-left-sm" style="padding-bottom:10px;">{{trans('messages.driver_dashboard.pay_period')}}</small>
  <select class="pay-period" id="pay_period" ng-init="pay_period='all'" ng-model="pay_period" ng-change="getPayment(undefined, 'pay_period')">
    <option value="current"><i class="icon-chevron-right"></i>{{trans('messages.driver_dashboard.current_statement')}}</option>
    ...
    <option value="all">{{trans('messages.driver_dashboard.all_statement')}}</option>
  </select>
</div>
</div>
...
<label class="" style="padding:6px 0px;" ng-init="completed_trips={{ $completed_trips }}" ng-model="completed_trips"> @{{ completed_trips }} </label>
<small style="padding:6px 0px;">{{trans('messages.driver_dashboard.completed_trips')}}</small>
</div>
<div class="col-lg-4 col-md-3 col-sm-3 col-xs-6 bor-left">
<label class="" style="padding:6px 0px;" ng-init="acceptance_rate='{{ $acceptance_rate }}';" ng-model="acceptance_rate"> {{ $acceptance_rate }} </label>
<small style="padding:6px 0px;">{{trans('messages.driver_dashboard.acceptance_rate')}}</small>
</div>
<div class="col-lg-4 col-md-3 col-sm-3 col-xs-6 bor-left" style="border-right:0px !important;">
<label class="" style="padding:6px 0px;" ng-init="cancelled_trips={{ $cancelled_trips }}" ng-model="cancelled_trips"> @{{ cancelled_trips }} </label>
<small style="padding:6px 0px;">{{trans('messages.driver_dashboard.cancelled_trips')}}</small>
</div>
</div>

<h2 class="flush-h2 pull-left" style="padding: 0px 20px;
">{{trans('messages.driver_dashboard.daily_earnings')}}</h2>
...
{{trans('messages.driver_dashboard.status')}}
<select style="font-size: 12px;padding: 9px 6px;margin: 0px 10px;" ng-model="earning_period" ng-init="earning_period='all_trips'" id='earning_period' ng-change="getPayment()">
    <option value="all_trips">{{trans('messages.driver_dashboard.all_trips')}}</option>
    <option value="completed_trips">{{trans('messages.driver_dashboard.completed_trips')}}</option>
    <option value="cancelled_trips">{{trans('messages.driver_dashboard.cancelled_trips')}}</option>
</select>
    ...
    <div class="pull-left full-width border-full earning_period_details" style="margin-bottom:20px;">
        <table style="font-size: 14px !important;" class="col-sm-12 table-bordered table-striped table-condensed cf" ng-init="all_trips={{ $all_trips }};currentPage=all_trips.current_page;totalPages=all_trips.last_page">
            <tbody>
                <tr>
                    <th>
                        {{trans('messages.driver_dashboard.pickup_time')}}
                    </th>
                    <th>
                        {{trans('messages.driver_dashboard.vehicle')}}
                    </th>
                    <th>
                        {{trans('messages.driver_dashboard.duration')}}
                    </th>
                    <th>
                        {{trans('messages.driver_dashboard.distance')}}
                    </th>
                    <!-- <th>
                        Cash Collected
                    </th> -->
                    <th>
                        {{trans('messages.driver_dashboard.total_earnings')}}
                    </th>
                </tr>
            </tbody>
                <tr ng-repeat="trips in all_trips.data">
                    <td>
                       @{{ trips.begin_date }} @{{ trips.pickup_time }}
                    </td>
                    <td>
                        @{{ trips.vehicle_name }}
                    </td>
                    ...
                    <td>
                        <span ng-bind-html="trips.currency.original_symbol"></span>&nbsp;
                        @{{ trips.driver_payout }}
                    </td>
                </tr>
                <tr >
