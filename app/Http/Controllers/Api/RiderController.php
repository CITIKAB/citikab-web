 ...
 */
	...
	/**
	 * Display the glade_pay_key details
	 *@param  Get method request inputs
	 *
	 * @return Response Json
	 */

	public function glade_pay_key(Request $request) {
		$user_details = JWTAuth::parseToken()->authenticate();
		...
		if($customer_id)
		{
			$card_brand = $customer_id->brand?$customer_id->brand:'';

			$card_last_digits = $customer_id->last4?$customer_id->last4:'';
			$token = $customer_id->token?$customer_id->token:'';
		}
		$user = array(
			'status_code' => '1',
			...
			'token'     =>  $token,
			'merchant_id' =>  $payment[0]->value,
			'merchant_key' => $payment[1]->value,
			'glade_pay_url' => $payment[2]->value,
			'is_live' => (false === strpos($payment[2]->value, 'demo' ) ),
		);
		return $user;
	}
 ...
 */
	...
	public function sosalert(Request $request) {
		...
		if (count($contact_details) > 0) {
			...
			foreach ($contact_details as $details) {
				$sms_response = $this->request_helper->send_nexmo_message('+'.$details->mobile_number, $message);
			}

			$this->request_helper->send_nexmo_message($mobile, $message);
			return response()->json(['status_message' => 'Success', 'status_code' => '1','sms_response'=>$sms_response]);
		} else {
			$this->request_helper->send_nexmo_message($mobile, $message);
	...
	{
		$driver_supported_versions = array('2.0','1.9');
		$rider_supported_versions = array('2.0','1.9');
