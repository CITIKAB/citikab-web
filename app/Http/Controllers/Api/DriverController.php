...
class DriverController extends Controller {
	...
	public function get_payout_details() {
		...
		$payout_details = BankDetail::select('id','user_id','holder_name','account_number','bank_name','bank_location','is_default')->where('user_id', $user->id)->get();
	...
	public function payout_changes(Request $request) {
		...
		$rules = array(
			...
			'payout_id' => 'required|exists:bank_details,id',
		...
		$check_user = BankDetail::where('id', $request->payout_id)
		...
		if ($request->type == 'default') {
			$payout = BankDetail::where('id', $request->payout_id)->first();
			...
			if ($payout->default == 'Yes') {
				...
				$payout_all = BankDetail::where('user_id', $driver->id)->update(['is_default' => 'no']);
		...
		if ($request->type == 'delete') {
			...
			if (strtolower($payout->is_default) == 'yes') {
				return response()->json(
					[
	...
	public function driver_bank_details(Request $request) {
        ...
        {
    		$old_bank_detail = BankDetail::where('user_id',$user->id)->first();
    		// if($bank_detail==null){
    		// }
        ...
        $all_bank_details = BankDetail::select('user_id','holder_name','account_number','bank_name','bank_location','is_default')->where('user_id', $user->id)->get();
