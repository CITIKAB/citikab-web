    ...
    public function index()
    {
        ...
        if(LOGIN_USER_TYPE=='company') {  //if login user is company then revenue calculated from company trips
            ...
            $data['today_revenue'] = Trips::whereDate('created_at', '=', date('Y-m-d'))
                ...
                ->get();
            $data['today_revenue'] = $data['today_revenue']->sum('company_earning');
        }else{
            /*$data['today_revenue'] = floatval( 
            Trips::whereDate('created_at', '=', date('Y-m-d'))
            ->where('status','Completed')
        ...
        if(LOGIN_USER_TYPE=='company') {   //if login user is company then revenue is sum of trip amount
            ...
            $data['total_revenue'] = $data['total_revenue']->sum('company_earning');
        ...
        $quarter_amount[1]=floatval($quarter1_chart->wherein(DB::raw('MONTH(created_at)'),$quarter1)->get()->sum(LOGIN_USER_TYPE=='company'?'company_earning':'commission'));
        $quarter_amount[2]=floatval($quarter2_chart->wherein(DB::raw('MONTH(created_at)'),$quarter2)->get()->sum(LOGIN_USER_TYPE=='company'?'company_earning':'commission'));
        $quarter_amount[3]=floatval($quarter3_chart->wherein(DB::raw('MONTH(created_at)'),$quarter3)->get()->sum(LOGIN_USER_TYPE=='company'?'company_earning':'commission'));
        $quarter_amount[4]=floatval($quarter4_chart->wherein(DB::raw('MONTH(created_at)'),$quarter4)->get()->sum(LOGIN_USER_TYPE=='company'?'company_earning':'commission'));
